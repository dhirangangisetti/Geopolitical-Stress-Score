...
st.markdown(
    "<div class='subtitle'>A weight-free, cross-asset stress gauge covering 25 years of geopolitical shocks.</div>",
    unsafe_allow_html=True,
)
...
"crises," but most don't move cross-asset risk in a measurable way — and a few
...
market stress.
<br><br>
<b>The score surfaces events that weren't on the front page.</b> The Turkey
...
# ── Hero chart (static, composite + top 5 events) ──────────────────────────────
...
# Alternate arrow directions so top-5 labels don't overlap
...
hero.update_layout(
    ...
    yaxis=dict(title='Composite GSS',
               showgrid=True, gridcolor='#21262d',
               zeroline=True, zerolinecolor='#30363d'),
)
st.plotly_chart(hero, use_container_width=True)
st.caption("Full window, Composite GSS only, top-5 events labelled. This is the headline view — no filters, no asset selector.")
...
to each era's volatility regime — 2008 stress is scored against 2007 conditions,
not against 2020.
<br><br>
...
assets are sign-flipped so every asset's contribution reads the same way:
positive contribution = stress, negative = calm.
<br><br>
...
judgment-weighted importance, no regime-specific weights.
<b>Event matching.</b> Events are matched to the next available trading day
on or after the event date (so a Saturday event is attributed to the following
Monday). Maximum lookahead is 10 trading days, which cleanly handles weekends
and extended market closures like the 9/11 trading halt.
</div>
""", unsafe_allow_html=True)
...
st.caption(
    "Direction is applied before averaging into the composite. "
    "A +1 asset with a high z-score and a −1 asset with a low z-score both contribute positively to the GSS."
)
...
# ═══════════════════════════════════════════════════════════════════════════════
# LAYER 2 — EVENTS TABLE WITH FILTERS
# ═══════════════════════════════════════════════════════════════════════════════

st.markdown("## Events table")
st.markdown(
    "<div class='section-hint'>Every scored event in the database. Filter by category or search by name. This table is independent of the chart controls.</div>",
    unsafe_allow_html=True,
)

tf1, tf2, tf3 = st.columns([2, 2, 1])
with tf1:
    table_cats = st.multiselect(
        "Category",
...
with tf2:
    name_search = st.text_input(
        "Search event name",
        value='',
        placeholder='e.g. Lehman, COVID, Turkey…',
        key='table_search',
    )
with tf3:
    table_sort = st.selectbox(
        "Sort",
        ['GSS score ↓', 'Date — newest first', 'Date — oldest first'],
        key='table_sort',
    )
...
if table_cats:
    ...
    table_src = table_src.iloc[0:0]  # empty if no category selected
if name_search.strip():
    mask = table_src['event_name'].str.contains(name_search.strip(), case=False, na=False)
    table_src = table_src[mask]

if table_src.empty:
    st.info("No events match the current filters.")
else:
    # Sort before string-formatting the date so ordering stays correct
    if table_sort == 'GSS score ↓':
...
st.markdown(
    "<div class='section-hint'>Asset selector, date range, category filter, and overlay toggles live in the sidebar. These controls affect the chart below only.</div>",
    unsafe_allow_html=True,
)

# Sidebar controls only the Layer 3 chart
with st.sidebar:
    st.markdown("### Chart controls")
    st.caption("These controls drive the interactive chart in Section 3. Layer 1 and Layer 2 are independent of these.")
    st.markdown("---")
    ...
    st.markdown(
        ...
        "Source: Yahoo Finance via yfinance<br>"
        "Rolling window: 252 trading days<br>"
        "Assets: 11 (equal weight)<br>"
        "Events: 31 scored (2001-2024)"
        "</div>",
        unsafe_allow_html=True,
    )
...
# Chart - reference band rectangles removed for cleaner background
fig = go.Figure()
...
if show_reflines and not series.empty:
    fig.add_hline(
        y=1.0,
        line=dict(color='rgba(255,166,87,0.45)', width=1, dash='dot'),
        annotation_text='Elevated', annotation_position='right',
        annotation=dict(font=dict(size=10, color='#ffa657'),
    ...
    fig.add_hline(
        y=2.0,
        line=dict(color='rgba(248,81,73,0.55)', width=1, dash='dot'),
        annotation_text='Crisis', annotation_position='right',
        annotation=dict(font=dict(size=10, color='#f85149'),
                        bgcolor='rgba(13,17,23,0.7)'),
    )
...
if is_composite and asset_counts is not None and not series.empty:
    fig.add_trace(go.Scatter(
        x=asset_counts.index, y=asset_counts.values,
        mode='lines',
        name='Assets in composite',
        line=dict(color='rgba(255,255,255,0.08)', width=1),
        yaxis='y2',
        hovertemplate='Assets in composite: %{y}<extra></extra>',
        showlegend=False,
    ))
    ...
    for category in ALL_CATEGORIES:
        ...
        for _, row in cat_ev.iterrows():
            ...
            hovers.append(
                ...
                f"GSS on event: {row['GSS_on_event']:.3f}"
                f"{closed_note}"
            )
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            name=category,
            marker=dict(symbol='diamond', size=9, color=colour,
        ...
        if show_eventline:
            for d in dates:
                fig.add_vline(
                    ...
                    line=dict(color=colour, width=0.7, dash='dash'),
                    opacity=0.3,
                )

y_label = 'Composite GSS Score' if is_composite else f'{selected_asset} - Z-Score (direction-adjusted)'
...
st.caption(
    ...
    "Composite uses all available assets on each date; assets with no data for a given period are excluded."
